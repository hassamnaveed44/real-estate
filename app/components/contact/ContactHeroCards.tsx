...
export default function ContactHero() {
  const contactCards = [
    {
      icon: <Mail className="w-[34px] h-[34px] text-[#A685FA]" />,
      label: 'info@estatein.com',
      link: 'mailto:info@estatein.com',
    },
    {
      icon: <Phone className="w-[34px] h-[34px] text-[#A685FA]" />,
      label: '+1 (123) 456-7890',
      link: 'tel:+11234567890',
    },
    {
      icon: <MapPin className="w-[34px] h-[34px] text-[#A685FA]" />,
      label: 'Main Headquarters',
      link: '#',
    },
    {
      icon: <Globe className="w-[34px] h-[34px] text-[#A685FA]" />,
      isSocial: true,
      socialLinks: [
        { name: 'Instagram', url: '#' },
        { name: 'LinkedIn', url: '#' },
  ...
  return (
    <section className="w-full bg-[#1a1a1a] border-b border-[#262626]">
      {/* Text Container - Reduced left/right padding via responsive classes */}
      <div 
      ...
      <div className="bg-[#141414] border-t border-[#262626] p-3 md:p-5 shadow-[0_0_0_4px_#191919] md:shadow-[0_0_0_6px_#191919] lg:shadow-[0_0_0_10px_#191919]">
        <div className="max-w-[1550px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
          {contactCards.map((card, index) => (
            <div 
              key={index}
              className="relative bg-[#1A1A1A] border border-[#262626] rounded-[10px] lg:rounded-[12px] py-[30px] lg:py-[40px] px-4 lg:px-5 flex flex-col items-center justify-between group transition-all duration-300 hover:border-[#333] overflow-hidden"
            >
              {/* Top-Right Arrow Icon */}
              <div className="absolute top-5 right-5 text-[#999] group-hover:text-white transition-colors">
                <ArrowUp className="w-[26px] h-[26px] lg:w-[34px] lg:h-[34px] rotate-45" />
              </div>

              {/* Icon Pill (Double Ring Effect) */}
              <div className="p-2 lg:p-[10px] rounded-full border border-[#A685FA]/30 mb-6 bg-[#1A1A1A]">
              ...
              {card.isSocial ? (
                <div className="flex items-center justify-center gap-[14px] lg:gap-[18px] mt-auto w-full">
                  {card.socialLinks.map((social, sIdx) => (
                    <a 
                      key={sIdx}
                      href={social.url}
                      className="text-[13px] md:text-[14px] lg:text-[16px] font-semibold text-white underline underline-offset-4 hover:text-[#A685FA] transition-colors whitespace-nowrap"
                    >
                      {social.name}
                    </a>
                  ))}
                ...
                <a 
                  href={card.link}
                  className="text-[14px] lg:text-[18px] font-semibold text-white text-center hover:text-[#A685FA] transition-colors mt-auto truncate max-w-full"
                >
                  {card.label}
                </a>
              )}
